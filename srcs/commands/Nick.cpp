...
/*   Updated: 2023/02/25 12:36:30 by gpernas-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{	
	...
	if (server->getPassword().size() > 0 && user.getPassword() != server->getPassword())
		return ;
	if (this->args.size() < 1)
		return setError(ERR_NONICKNAMEGIVEN, *server, user, 0);
	...
	for (size_t i = 0; i != this->args.size(); i++)
		if (!std::isprint(this->args.at(i)) || this->args.at(i) == '#')
			return setError(ERR_ERRONEUSNICKNAME, *server, user, 1, this->args.c_str());
	for (std::vector<IRC::User>::iterator it = server->getUsers().begin(); it != server->getUsers().end(); it++)
		if (this->args.compare(it.base()->getNick()) == 0)
			return setError(ERR_NICKNAMEINUSE, *server, user, 1, this->args.c_str());
